...
- (void)didLoadFromCCB{
    _backgrounds1 = @[_background1_1, _background1_2];
    _backgrounds2 = @[_background2_1, _background2_2];
...
-(void) loopSprites:(NSArray*)array{
    ...
    for (CCNode *currentSprite in array) {
        ...
        if (groundScreenPosition.x <= (-1 * currentSprite.contentSize.width)) {
            currentSprite.position = ccp((currentSprite.position.x + [array count] * currentSprite.contentSize.width)-4, currentSprite.position.y);
        }
    }
}
